...
public class UsuarioFeedDAO extends DAO<UsuarioFeed> {
    ...
    public UsuarioFeed retornaUsuarioFeed(Integer idUsuario, Integer idFeed) {
        ...
        try {
            TypedQuery<UsuarioFeed> query = manager.createQuery("select u from UsuarioFeed u where u.usuario.id = :idUsuario AND u.feed.id = :idFeed", UsuarioFeed.class);
    ...
    public List<UsuarioFeed> usuarioFeedsTipo(Integer idUsuario, TipoFeed tipoFeed) {
        EntityManager manager = JpaUtil.getEntityManager();
        try {
            String select = "select u from UsuarioFeed u where u.usuario.id = :idUsuario";
            switch (tipoFeed) {
                case NAOLIDO:
    ...
    public List<UsuarioFeed> usuarioFeedsTag(Integer idUsuario, Integer idTag) {
        EntityManager manager = JpaUtil.getEntityManager();
        try {
            TypedQuery<UsuarioFeed> query = manager.createQuery("select u from UsuarioFeed u JOIN FETCH u.listaTag lt where u.usuario.id = :idUsuario AND lt.id = :idTag", UsuarioFeed.class);
            query.setParameter("idUsuario", idUsuario);
    ...
    public List<UsuarioFeed> compartilhadosPeloUsuario(String email) {
        EntityManager manager = JpaUtil.getEntityManager();
        try {
            TypedQuery<UsuarioFeed> query = manager.createQuery("select u from UsuarioFeed u "
